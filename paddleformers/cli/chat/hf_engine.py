...
if TYPE_CHECKING:
    ...
    from ...datasets.template import Template
    from ..hparams import (
        DataArguments,
        FinetuningArguments,
        GeneratingArguments,
...
class HuggingfaceEngine(BaseEngine):
    def __init__(
        ...
        self.can_generate = model_args.stage.lower() == "sft"
        ...
        self.model = AutoModelForCausalLM.from_pretrained(model_args.model_name_or_path, dtype="bfloat16").eval()
        self.generating_args = asdict(generating_args)
        try:
    ...
    def _process_args(
        ...
        mm_inputs = template.mm_plugin.get_mm_inputs(
            ...
            mm_input_dict["imglens"],
            mm_input_dict["vidlens"],
            mm_input_dict["audlens"],
            None,
            processor,
        )
        messages = template.mm_plugin.process_messages(
            messages, mm_input_dict["images"], mm_input_dict["videos"], mm_input_dict["audios"], mm_inputs, processor
        )
        ...
        inputs = paddle.tensor([prompt_ids], device=model.device)
        attention_mask = paddle.ones_like(inputs, dtype=paddle.long)
        ...
        for key, value in mm_inputs.items():
            if isinstance(value, list) and isinstance(value[0], paddle.Tensor):  # for pixtral inputs
                value = paddle.stack(value)  # assume they have same sizes
            elif (
                isinstance(value, list) and isinstance(value[0], list) and isinstance(value[0][0], paddle.Tensor)
            ):  # for minicpmv inputs
                value = paddle.stack([paddle.stack(v) for v in value])
            elif not isinstance(value, paddle.Tensor):
                value = paddle.tensor(value)

            if paddle.is_floating_point(value):  # cast data dtype for paligemma
